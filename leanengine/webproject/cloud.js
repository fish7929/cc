            ...
            try {
              requestClient(ticket_url + '?access_token=' + access_token + '&type=jsapi', function (error2, res2, callData) {
                if (!error && res.statusCode == 200) {
                  ...
                  //生成签名方法 
                  var signature = calcSignature(JSON.parse(callData).ticket, createNonceStr(), createTimeStamp(), _url);
                  console.log("signature>>>" + signature);

                  console.log(">>>" + index + ">>>token:" + access_token + "|ticket:" + JSON.parse(callData).ticket + "|signature:" + signature);
